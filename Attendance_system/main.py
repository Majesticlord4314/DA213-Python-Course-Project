...
# attendance_data=pd.read_csv('student.csv')
current_date = datetime.now().strftime("%Y-%m-%d")
file_name = f"data_{current_date}.csv"
if os.path.isfile(file_name):
    # Load the existing data from the CSV file
    attendance_data = pd.read_csv(file_name)
...
cap = cv2.VideoCapture(1)  # Use 0 or -1 for default camera
...
while True:
    ...
    if counter!=0:
        ...
        if counter==1:
            ...
            image_value = data.loc[data['Id'] == id, 'image_name'].values[0]
            ...
            if secondsElapsed > 600:
                ...
                for i in range(len(attendance_data)):
                    if attendance_data["id"][i]==id:
                        attendance_data["marked"][i]=1
                        current_time = datetime.now().strftime("%H:%M:%S")
                        attendance_data["time"][i]=current_time
            else:
                modeType = 3
                counter = 0
                imgBackground[44:44 + 633, 808:808 + 414] = imgModeList[modeType]
...
attendance_data.to_csv(file_name,index=False)
